    ...
    const pickImage = async () => {
      ...
      async function uploadImageAsync(uri, checkId, key) {
        ...
        if(uri){
          ...
          const result = await  firebase.default.storage().ref('Avatar/'+uid22).put( blob).then(()=>{
      ...
      <View style={[styles.container, { paddingTop: inset.top }]}>
        ...
        <ScrollView>
          <View style={{ paddingHorizontal: 16, paddingTop: 20 }}>
              <TouchableOpacity  style={{ width: 90, height: 90, borderRadius: 90 ,alignSelf:'center' }}  
              onPress={() =>{pickImage()}}>
              <Image
          style={{ width: 90, height: 90, borderRadius: 90 ,alignItems: 'center' }}
          resizeMode="contain"
          source={{
            uri: image ? image : firebase.default.auth()?.currentUser?.photoURL
